...
namespace ddafa
{
    namespace cuda
    {
        ...
        auto Preloader::expand(input_type img) -> input_type
        {
            ...
            auto add_top = scheduler.get_additional_projection_lines_top();
            auto add_bot = scheduler.get_additional_projection_lines_bot();
        ...
        auto Preloader::split(input_type img) -> void
        {
            ...
            for(auto d = 0; d < devices_; ++d)
            {
                ...
                for(auto i = 0u; i < subproj_num; ++i)
                {
                    ...
                    auto row_num = lastRow - firstRow + 1;
                    ...
                    remaining_[d][i].emplace_back(extract(img, firstRow, row_num));
                }
            }
        }
