...
namespace paris
{
    namespace cuda
    {
        ...
        auto make_volume_host(std::uint32_t dim_x, std::uint32_t dim_y, std::uint32_t dim_z) -> volume_host_type
        {
            ...
            return volume_host_type{std::move(ptr), dim_x, dim_y, dim_z, 0u, cuda_stream{}};
        }
        ...
        auto make_volume_device(std::uint32_t dim_x, std::uint32_t dim_y, std::uint32_t dim_z) -> volume_device_type
        {
            ...
            return volume_device_type{std::move(ptr), dim_x, dim_y, dim_z, 0u, cuda_stream{}};
        }
        ...
        {
            glados::cuda::copy(glados::cuda::async, d_v.buf, h_v.buf, d_v.meta.stream, h_v.dim_x, h_v.dim_y, h_v.dim_z);
            d_v.off = h_v.off;
        }

        auto copy_d2h(const volume_device_type& d_v, volume_host_type& h_v) -> void
        {
            glados::cuda::copy(glados::cuda::async, h_v.buf, d_v.buf, d_v.meta.stream, d_v.dim_x, d_v.dim_y, d_v.dim_z);
            glados::cuda::synchronize_stream(d_v.meta.stream);
            h_v.off = d_v.off;
        }
    }
}
