...
namespace paris
{
    ...
    {
        auto err = cudaSetDevice(device_); // glados::cuda::set_device() can throw, cudaSetDevice() does not
        if(err != cudaSuccess)
        {
    ...
    auto preloader_stage::run() -> void
    {
        ...
        try
        {
            ...
            while(true)
            {
                ...
                auto stream = glados::cuda::create_concurrent_stream();

                // initialize the destination data before copying
                glados::cuda::fill(glados::cuda::async, dev_p, 0, stream, p.width, p.height);
                glados::cuda::copy(glados::cuda::async, dev_p, p.ptr, stream, p.width, p.height);

                glados::cuda::synchronize_stream(stream);
